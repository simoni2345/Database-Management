...
/*
Left and Right outer joins:  A left outer join retains all of the rows of the “left” table, regardless of whether there is a row that matches on the “right” table. This means that NULLs are displayed. The “left” table is the table that is written first in the SQL join statement. Right outer join is pretty much the same thing as a left outer join, except that the row from the right table are displayed in the result of the query, regardless of whether or not they have matching values in the left table. 
